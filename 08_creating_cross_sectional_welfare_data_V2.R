...
lm_income <- readRDS("working/models/lm_income_w20.RDS")
...
pop <- read_csv("data/population_change.csv")
...
manuf <- read_csv("data/2020_industry_employment.csv")
...
df <- df %>% 
  select(id, immig_burden, la_code, male, uni, white_british,  
...
save(df, file = "working/data/cross_sectional_df_welfare.RData")
